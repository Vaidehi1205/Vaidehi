...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    """
    pro = temperature*neutrons_emitted
    if temperature<800 and neutrons_emitted>500 and pro<500000:
        return True
    else:
        return False
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    """
    threshold_cal = temperature * neutrons_produced_per_second
    
    lower_bound = 0.9 * threshold
    upper_bound = 1.1 * threshold

    if threshold_cal < lower_bound:
        return "LOW"
    elif lower_bound <= threshold_cal <= upper_bound:
        return "NORMAL"
    else:
        return "DANGER"
